...
public class EmbedGenerator
{
	...
	public EmbedGenerator(MessageEmbed message)
	{
		...
		try
		{
			for(Field selectedField : message.getFields()) embed.addField(selectedField);
		}
	...
	public EmbedGenerator(MessageChannel channel, MessageEmbed message)
	{
		this.channel = channel;
		embed = new EmbedBuilder();
		try {embed.setAuthor(message.getAuthor().toString());}
		...
		try {embed.setThumbnail(message.getThumbnail().toString());}
		catch(Exception exception)
		{
			new ErrorHandler(exception);
		}
		...
		catch(Exception exception) {new ErrorHandler(exception);}
	}
