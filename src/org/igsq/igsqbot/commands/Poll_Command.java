...
public class Poll_Command {
	...
	private void poll()
	{
		if(args.length >= 3) 
		{
			...
			new EmbedGenerator(channel).title("Poll:").text(topic).element("Options:", options).footer("Poll created by "+ author.getAsTag()).thumbnail(author.getAvatarUrl()).color(guildAuthor.getColor()).reaction(reactions).send();
		}
